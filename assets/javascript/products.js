const CSV_REST  = "restaurants.csv";
const CSV_PROD  = "products.csv";
...
async function cargarAmbos() {
    try {
        ...
        setLoadMsg("⚠ " + err.message +
            " — Ejecuta los scripts Python y recarga la página.");
    }
}
...
function parsearFila(linea) {
    ...
    for (let i = 0; i < linea.length; i++) {
        ...
        else if (ch === "," && !enComillas) { res.push(actual); actual = ""; }
        else { actual += ch; }
    }
    res.push(actual);
    return res;
...
function cargarRestaurante(id) {
    ...
    restActual   = todosRest.find(r => String(r.id_restaurante).trim() === idStr) || null;
    ...
    document.getElementById("bannerNombre").textContent = restActual.nombre   || "—";
    document.getElementById("bannerTipo").textContent   = restActual.tipo     || "—";
    document.getElementById("bannerDist").textContent   = restActual.distrito || "—";
    document.getElementById("bannerRating").textContent = "★ " + (restActual.rating || "—");
...
function filtrarProd() {
    const busqueda = (document.getElementById("searchProd").value || "").toLowerCase();
...
function actualizarStats(lista) {
    ...
    document.getElementById("statMin").textContent   = "S/ " + Math.min(...precios).toFixed(2);
    document.getElementById("statMax").textContent   = "S/ " + Math.max(...precios).toFixed(2);
    document.getElementById("statProm").textContent  = "S/ " + (sum / precios.length).toFixed(2);
}
