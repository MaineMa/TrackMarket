    ...
    fetch(CSV_RESTAURANTES)
        .then(res => {
            if (!res.ok) throw new Error(
                "No se encontró '" + CSV_RESTAURANTES + "'. " +
...
function inicializar() {
    todosLosRest.forEach(r => { if (r.tipo) tiposUnicos.add(r.tipo); });
...
function filtrar() {
    ...
    const minRating = parseFloat(document.getElementById("filtroRating").value) || 0;
